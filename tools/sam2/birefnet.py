...
class _Ops:
    """comfy.ops.manual_cast 的替代。原本只用到這四個，而 comfy 那邊
    的實作就是 nn 的薄包裝，直接用 nn 行為一致。"""
    Conv2d = nn.Conv2d
    Linear = nn.Linear
...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous().to(dtype=attn.dtype, device=attn.device)  # nH, Wh*Ww, Wh*Ww
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, H, W):
        ...
        # dtype 要跟 x 一致：comfy 那邊的 manual_cast 會把權重追著輸入轉，
        # 直接用 nn 時沒人代勞，漏掉就會把 fp16 的 attn 推回 fp32。
...
class DeformableConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        weight = self.regular_conv.weight.to(dtype=x.dtype, device=x.device)
...
class BiRefNet(nn.Module):
    def __init__(self, config=None, dtype=None, device=None, operations=ops):
